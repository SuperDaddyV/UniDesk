...
UniDesk は、Windows デスクトップの端に常駐する軽量でカスタマイズしやすいサイドバーです。時刻と天気、ハードウェア監視、モデルレーダー、ショートカット、ToDo、クイックメモ、クイックテキストを、ひとつの使いやすいデスクトップ作業スペースにまとめます。
...
- 「総合トップ」と「コストパフォーマンス推薦」の判断カードを先に表示し、総合／バックエンド／フロントエンド／知識の Top 5 参考リストを続けて表示します。
- データは [ModelDial Radar](https://modeldial.com/radar) の公開評価（[CC BY 4.0](https://modeldial.com/data-license)）から取得します。「コストパフォーマンス推薦」は公開元の公式 `value` タグだけを使い、UniDesk が得点や費用を再計算することはありません。
- 新規インストールでは既定で無効で、無効時はネットワーク通信を行いません。UniDesk の実行中にモジュールを有効にすると、ローカルキャッシュを先に読み、`https://modeldial.com/api/v1/radar/latest.json` から 6 時間に 1 回を上限として自動更新を確認します。手動更新も可能です。
- 読み取り専用の判断材料です。UniDesk はモデルを実行せず、ローカル評価やモデルツール設定の変更も行いません。公開評価データは提携や推奨を意味せず、実際の性能はアカウント、ルート、エンドポイントによって異なる場合があります。
...
二つの判断カードで現在の総合トップと ModelDial 公式のコストパフォーマンス推薦を先に確認し、その後でタスク別の Top 5 を参照できます。

![UniDesk モデルレーダー紹介画像](images/unidesk-model-radar.png)

紹介画像は 2026 年 8 月 30 日の公開バッチを使用しています。アプリ内の結果は ModelDial Radar のデータ更新に追従します。
...
- 総合トップ、公式のコストパフォーマンス推薦、総合／バックエンド／フロントエンド／知識の Top 5 を表示するモデルレーダー。
- モジュールの表示 / 非表示と並び替え。
