...
`v2.2.1` 正式安装包的官方分发入口是 [v2.2.1 GitHub Release](https://github.com/SuperDaddyV/UniDesk/releases/tag/v2.2.1)。该版本采用仅适用于此精确版本的未签名正式发布例外；安装包为 `Authenticode: NotSigned`，Windows 可能显示 SmartScreen 或企业策略提示。请从官方 Release 下载并核对 SHA-256。安装包文件名为：
...
脚本会将主程序、硬件服务和修复工具发布到全新的版本目录，再从这些确定输入编译安装包。干净工作区生成的未签名 `v2.2.1` 制品必须通过 `Test-UnsignedReleaseReadiness.ps1`，完成适用人工矩阵并由项目所有者最终批准后才能公开；SignPath 签名工作流继续保留给未来签名版本，且不会自动创建 GitHub Release。
...
公开发布包遵循[代码签名政策](CODE_SIGNING_POLICY.md)和[隐私政策](PRIVACY.md)。`v2.2.1` 采用单独批准、仅适用于该精确版本的未签名正式版例外：必须来自精确公开提交，通过人工、完整载荷和 SHA-256 校验，并清晰标注 `Authenticode: NotSigned` 及 Windows 可能显示的 SmartScreen／企业策略提示。
