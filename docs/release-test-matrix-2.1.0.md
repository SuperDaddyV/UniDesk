...
| I-20 | `Startup=true`，HKCU Run 中存在 `UniDesk` 旧值：分别指向明确不存在的绝对 `UniDesk.exe`、仍存在的其它目录 `UniDesk.exe`、畸形命令和名称不匹配目标 | 只替换目标明确不存在且命令及文件名严格合法的失效旧值；目标存在、无法确认存在性、命令畸形或名称不匹配时拒绝覆盖并保留原值；升级首次启动不再因已删除的旧安装目录弹出写入失败 |
...
- [x] 使用仓库钉住的 SDK `10.0.302` 运行 `dotnet test UniDesk.sln -c Release --no-restore`：`590/590` 通过，`0` 跳过；正式提交仍须由 GitHub CI 复核。
- [x] Release 构建零警告、零错误。
...
- 覆盖目录盘符根与失效开机启动项修复加入后，当前工作区使用仓库钉住的 SDK `10.0.302` 完成全量测试 `590/590` 通过，`0` 跳过；正式提交仍须由 GitHub CI 复核。
