...
| I-15 | 分别在系统 `{commonpf}` 或其直属 Program Files 父目录赋予普通用户可删除子项／改 ACL 的有效权限后启动安装；另保持这两级安全但把主程序选择到普通 D／E 盘目录 | 前两种场景均在复制系统组件和创建卸载器前明确拒绝，且不修改父目录 ACL；后一场景允许继续，主程序安装到所选盘，系统组件仍位于 `{commonpf}\UniDesk`；盘符根目录的宽松但不向下生效权限不单独导致误拒绝 |
| I-16 | 全新安装时选择 `D:\Apps\UniDesk` 等尚不存在或为空的本地目录，完成后再次覆盖安装 | 首次安装保留所选位置；覆盖安装默认沿用该位置并允许直接继续，用户数据保持不变 |
| I-17 | 分别选择网络路径、磁盘根目录、经过 junction／符号链接的路径、含同名假 `UniDesk.exe` 但未登记的非空目录 | 安装器在复制文件前停留在目录选择页并给出可理解提示，不覆盖未知目录、不递归修改主程序目录 ACL |
...
| U-03 | 同 AppId 旧版安装在 D／E 盘自定义目录，存在严格 owned 的旧目录硬件服务；不修改安装位置，直接覆盖 | 新安装器锁定并收紧旧目录，只删除注册路径内逐个验证的旧 `uninsNNN.exe／.dat／.msg`，不执行旧卸载器；随后禁用并确认停止旧服务，再从 `{commonpf}\UniDesk` 注册新版服务，不删除用户数据、不产生两个服务 |
| U-04 | 同 AppId 旧版位于自定义目录，但本次选择新的本地目录 | 新安装器完成同样的旧卸载器和系统组件迁移，并只清理严格指向旧目录且文件名匹配的 Run／计划任务；不执行旧卸载器、不整体删除旧目录，完成后提示确认新版后手动删除旧程序文件夹 |
| U-05 | 篡改同 AppId 的 `UninstallString` 使其指向旧目录外、非 `uninsNNN.exe` 名称，或让任一已确认旧卸载器文件无法删除 | 安装在复制新版文件和停止服务前中止；不执行、不删除外部路径，不用通配符扩大删除范围 |
...
- 当前电脑的旧主程序和旧 `LocalSystem` 服务位于弱 ACL 的 `D:\Program Files\UniDesk`。新版不再从该目录运行服务或卸载器：覆盖安装应沿用 D 盘主程序位置，同时把系统组件迁移到 `{commonpf}\UniDesk`。不得运行旧目录中的提权卸载器；签名候选仍需按 U-03 实装确认迁移结果。
