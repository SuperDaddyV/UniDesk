...
| I-15 | 在系统 `{autopf}` 或其卷根赋予普通用户可删除子项／改 ACL 的有效权限后启动安装；另在祖先链受保护的非系统盘 Program Files 环境测试 | 弱祖先 ACL 场景在复制文件和创建卸载器前明确拒绝，且不修改父目录 ACL；受保护的系统 Program Files 即使位于非系统盘也允许继续 |
...
| U-03 | 同 AppId 旧版安装在自定义弱父目录，存在严格 owned 硬件服务和旧 Run／计划任务启动项；直接运行新安装器 | 新安装器先禁用并确认停止旧服务，只清理严格指向旧目录且文件名匹配的启动项，再安装到 `{autopf}\UniDesk` 并接管卸载注册；全程不执行旧卸载器、不自动删除旧目录，完成后只提示可在资源管理器手动删除旧程序文件夹 |
| U-04 | 同 AppId 旧版位于自定义目录，但硬件服务不存在或已手工删除 | 仍进入同一受信迁移流程并清理严格 owned 启动项，不遗留两个开机启动版本；用户数据保留，旧目录仅由用户手动删除 |
...
- [x] 自动回归验证安装器最低版本、签名工作流完整 SHA、剪贴板披露、备份上限、收缩态排版、固定受保护安装目标、祖先 ACL 预检、旧路径受信迁移和服务实质停止约束。
...
- 当前电脑的 `D:\Program Files`／卷根和 `C:\` 祖先 ACL 异常宽松，新安装器会按设计在复制前拒绝。不得为通过测试放宽门禁，也不得运行弱目录中的旧提权卸载器；需先单独处理系统 ACL 或改用祖先权限正常的 Windows 测试环境，再执行 U-03 和签名候选实装。
