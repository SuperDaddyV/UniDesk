...
- 启用开机自启动：写入 Run 启动项
- 启用时若 `UniDesk` Run 值已经严格指向当前安装目录中的 `UniDesk.exe`，允许原位刷新；若现有命令可无歧义解析为绝对路径、文件名严格为 `UniDesk.exe`，且文件系统明确确认目标不存在，视为本产品遗留的失效启动值并允许替换。命令畸形、相对路径、文件名不匹配、目标仍存在或无法确定是否存在时必须拒绝覆盖，不能只凭通用值名称接管其它程序
- 禁用开机自启动：移除 Run 启动项
