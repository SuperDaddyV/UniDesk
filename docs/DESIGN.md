...
- 收缩态始终由专用的 `TimeWeatherModuleView` 迷你仪表盘承载，不受展开态模块排序影响；调整模块顺序后仍须显示时间、天气、硬件摘要与下一待办的固定组合。收缩窗口高度固定为 `190` 个逻辑像素，`TimeWeatherModuleView` 在收缩态固定为 `140` 个逻辑像素。工作区允许时，收缩态实际宽度不得低于 `350` 个逻辑像素；用户保存的展开宽度低于该值时只在收缩期间临时增宽，重新展开后必须恢复原保存宽度，且不得把临时宽度写回设置。模型雷达继续只占一行，使用 `TOP`／`VAL` 固定标签和中央低对比度竖分隔线划分两个推荐区；每个推荐区内部使用「标签｜模型配置｜分数」三列，标签、模型配置和分数均须完整显示，不得对模型配置使用省略号。`TOP`／`VAL` 标签使用 `Medium`，模型配置、分数及待办文字保持 `Normal`，不得使用 `SemiBold`。紧凑显示名允许把 `gpt-5.6-sol`／`max` 规范化为 `GPT5.6-Sol`／`Max`，但完整悬浮提示必须保留发布方原始模型 ID 和推理强度。缓存或过期标记随综合推荐的模型配置显示，不得吞掉模型名或分数。模型雷达推荐与下一待办均沿用上方时间天气区使用的主题 `BodyTextStyle`／`BodyFontFamily`，加载、不可用和无数据状态沿用 `CaptionTextStyle`；紧凑态不得引入等宽数据字体。模型雷达行与待办行均使用固定 `16` 个逻辑像素行盒和 `BlockLineHeight`，模型雷达标签、模型配置、分数、待办标题与到期文本必须共享相同字体、字号和行盒；只允许标签按上述规则使用 `Medium`，并允许前景色和水平对齐方式不同。界面必须在 `100%`、`125%`、`150%`、`175%`、`200%` DPI 及收缩态下限／当前保存／最大面板宽度下无裁切、重叠或越界。
- 模型雷达模块启用时，收缩态在下一待办上方显示一行只读决策摘要：综合项取当前快照 `overallRankings` 的第一项及其官方综合分；性价比项只取第一项带官方 `decisionTags: ["value"]` 的配置及其同批次综合分，不自行计算分数／费用比，也不得把 `lowest_cost` 冒充性价比。没有官方 `value` 标签时必须明确显示暂无官方性价比推荐。
- 收缩态摘要只能消费 `ModelRadarViewModel` 已有快照，不得因为面板收缩而启用模型雷达、读取缓存、启动计时器或发起网络请求。模型雷达模块关闭时整行隐藏；加载、待发布、不可用或模式不完整时显示对应状态而不伪造两个推荐。快照发布时间按当前本地日期判断：当天数据可标为今日，非当天或离线缓存必须显示实际日期／缓存状态，不能称为今日推荐。窄面板允许对可见文本做省略，但完整模型、推理强度、分数、批次时间和缓存状态必须保留在工具提示中。
