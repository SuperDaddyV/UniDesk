...
- 收缩态始终由专用的 `TimeWeatherModuleView` 迷你仪表盘承载，不受展开态模块排序影响；调整模块顺序后仍须显示时间、天气、硬件摘要与下一待办的固定组合。收缩窗口高度固定为 `206` 个逻辑像素，`TimeWeatherModuleView` 在收缩态固定为 `156` 个逻辑像素。工作区允许时，收缩态实际宽度不得低于 `320` 个逻辑像素；收缩态使用与展开态相同的最小宽度，不得为了摘要临时放宽窗口，重新展开后必须恢复用户保存的宽度，且不得把收缩态实际宽度写回设置。模型雷达使用等宽双列四宫格：左列表示综合推荐，右列表示性价比推荐；每列第一行将 `TOP`／`VAL` 标签与分数作为一个居中组合，第二行居中显示完整模型配置。仅在两列之间使用低对比度 `1 DIP` 竖向分隔线；两行之间不得绘制横线，也不得绘制四个独立边框。`TOP`／`VAL` 标签使用 `Medium`，模型配置、分数及待办文字保持 `Normal`，不得使用 `SemiBold`。紧凑态完整显示推理强度，不使用 `Med` 等缩写；界面显示名统一执行品牌与单词大小写规范化，例如把 `gpt-5.6-sol`／`medium` 显示为 `GPT5.6-Sol`／`Medium`，但不得修改 API 原值、缓存或排序键，完整悬浮提示必须保留发布方原始模型 ID 和推理强度。缓存或过期标记随综合推荐的模型配置显示，不得吞掉模型名或分数。模型雷达推荐与下一待办均沿用上方时间天气区使用的主题 `BodyTextStyle`／`BodyFontFamily`，加载、不可用和无数据状态沿用 `CaptionTextStyle`；紧凑态和展开态模型雷达均不得为模型名称、推理强度或分数引入等宽数据字体，列对齐由 Grid 负责。四宫格的两行和待办行均使用固定 `16` 个逻辑像素行盒和 `BlockLineHeight`，模型雷达标签、模型配置、分数、待办标题与到期文本必须共享相同字体、字号和行盒；只允许标签按上述规则使用 `Medium`，并允许前景色不同。界面必须在 `100%`、`125%`、`150%`、`175%`、`200%` DPI 及收缩态下限／当前保存／最大面板宽度下无裁切、重叠或越界。
- 模型雷达模块启用时，收缩态在下一待办上方显示一个四宫格只读决策摘要区域：综合项取当前快照 `overallRankings` 的第一项及其官方综合分；性价比项只取第一项带官方 `decisionTags: ["value"]` 的配置及其同批次综合分，不自行计算分数／费用比，也不得把 `lowest_cost` 冒充性价比。没有官方 `value` 标签时必须明确显示暂无官方性价比推荐。
- 收缩态摘要只能消费 `ModelRadarViewModel` 已有快照，不得因为面板收缩而启用模型雷达、读取缓存、启动计时器或发起网络请求。模型雷达模块关闭时四宫格摘要整体隐藏；加载、待发布、不可用或模式不完整时在四宫格区域显示对应状态而不伪造两个推荐。快照发布时间按当前本地日期判断：当天数据可标为今日，非当天或离线缓存必须显示实际日期／缓存状态，不能称为今日推荐。当前受支持的模型名称、完整推理强度和分数不得缩写或省略；未来外部数据出现超过最小宽度承载能力的异常超长标识时，才允许将视觉省略作为防越界兜底，且完整模型、推理强度、分数、批次时间和缓存状态必须保留在工具提示中。
...
- 服务禁用只能作为删除前的安全步骤，不能替代删除成功。安装、升级或卸载流程只有在再次查询确认 `UniDeskHardwareService` 已不存在后才能继续复制、重新注册或报告清理成功；删除命令失败、服务仍可查询或查询结果不确定时必须停止该方向并保留可诊断状态。
...
- `v2.1.0`、`v2.2.0` 与 `v2.2.1` 分别拥有一次由项目所有者明确批准、且只适用于该精确版本的未签名正式发布例外；这些例外均不构成签名通过，也不自动授权任何后续版本。未签名正式包必须从最终公开 `main` 的精确干净提交重新构建，通过锁定还原、零警告 Release 构建、全量测试、依赖漏洞、版本一致性、完整载荷清单和 SHA-256 门禁；`Test-UnsignedReleaseReadiness.ps1` 的允许列表只能精确包含已书面批准的版本，并必须验证安装包与全部 UniDesk 一方 PE 均为 `NotSigned`、源清单与当前提交一致、载荷无 PDB，并生成可公开核对的校验清单。门禁不得只信任调用方提供的清单状态：`isDirty` 必须存在且严格为布尔 `false`，门禁必须重新读取当前 `HEAD` 和包含未跟踪文件的工作区状态，并核对安装包版本资源中绑定的源清单 SHA-256；项目／锁文件枚举排除 `artifacts`、`bin`、`obj` 和 `publish` 时必须同时识别 Windows 反斜杠与规范化斜杠，忽略目录中的诊断工程不得改变正式门禁清单。只有当前源码、源清单、递归载荷哈希和安装包载荷指纹一致时才允许生成公开清单。README、发布说明和 GitHub Release 必须在下载与完整性说明中直接标注 `Authenticode: NotSigned`，并以简短、中性措辞说明 Windows 可能显示 SmartScreen／企业策略提示；不得隐藏签名状态，也绝不得把未签名制品描述为已签名或通过 SignPath。只有项目所有者对精确版本的最终确认才允许创建对应 Git tag 或 GitHub Release。
...
- `WeatherService.SetCityAsync` 必须先取消旧刷新并建立新的请求身份；只有城市、generation 和取消源仍与当前选择一致的请求可以写天气缓存或返回为当前城市结果，旧城市的迟到响应不得覆盖新选择。
...
#### 和风天气网络边界
- `QWeatherApiClient` 只向经过 `qweatherapi.com` 专属子域校验的 HTTPS 主机发送 `X-QW-Api-Key`；生产客户端必须禁用自动重定向，任何 3xx 都按失败处理，不能把 Header 或旧接口查询参数中的 Key 转发到重定向目标。
- 天气请求使用 `ResponseHeadersRead`，响应体按流读取且最多接受 1 MiB；`Content-Length` 已超限或流式读取超过上限时立即取消并按无效响应处理，不得为错误状态页或压缩后膨胀内容分配无界字符串。

#### UI 线程与节流
- Service 层不得直接操作 WPF 控件；仅返回模型或通过事件/回调通知 ViewModel
- ViewModel 仅在最终需要更新绑定属性时切回 UI 线程，避免在 UI 线程执行 I/O 或复杂计算
- `Microsoft.Data.Sqlite` 的异步 API 实际同步执行。`SettingsService.InitializeAsync` 必须在后台线程完成 SQLite 初始化并一次性预载全部设置，且在构造任何 WPF ViewModel 前完成；启动后的批量加载、隐私迁移、备份恢复和其它可能超过单条轻量查询的数据库操作也必须在明确的后台执行边界内完成，连接、命令和事务只能在该次后台操作内部创建和使用。同步 `GetSetting`／`GetValue` 只允许读取已初始化缓存。迁移或恢复使缓存失效后，异步流程必须先重新加载缓存，不能让后续 Dispatcher 调用回退到同步 SQLite 查询
...
- 日志只记录稳定操作名、错误类型和必要的非敏感状态；不得写入 API Key、精确坐标、剪贴板／便签／待办正文、完整快捷方式目标或启动参数。异常消息和堆栈在写入前必须清除可识别的本地绝对路径和 URI 查询凭据，不能依赖调用方逐处记得脱敏。
- 以管理员或 `LocalSystem` 运行的组件写入 `%ProgramData%\UniDesk\logs` 前，必须验证从 ProgramData 边界到日志文件的路径不存在重解析点，并把 UniDesk 日志目录 ACL 收紧为仅 `SYSTEM`／`Administrators` 可创建、追加、删除或改 ACL；不得通过可预测文件名跟随普通用户预置的硬链接、符号链接或 junction。
...
- **导入**：
  ...
  8. 校验阶段还必须验证业务记录的枚举值属于已定义域、剪贴板历史 `UseCount` 严格大于零、其它计数非负且不超过业务上限、文本片段分类不是空白字符串，并确认日期可解析且满足字段间约束；这些原始字段不得在应用计划时通过 clamp、默认枚举或默认日期静默改写。非空 `ModuleSettings` 必须在创建导入计划前完成 JSON 解析、稳定模块 ID 校验和目录规范化，格式错误或结构无效时拒绝整个备份；只有缺失或空值可以使用当前规范默认目录。快捷方式连续排序和合法 `ModuleSettings` 的目录补全仍属结构规范化，必须在预览与恢复中保持确定性
...
#### 界面与归属
- 顶部使用通用雷达风格图标，不使用 ModelDial Logo 或任何模型厂商 Logo；标题字号和样式与相邻模块一致，标题区只保留「模型雷达」和手动刷新按钮，不重复显示最新状态、批次或发布时间。两张纵向紧凑决策卡的主文案固定为「综合最高」和「性价比推荐」；模型名称与推理强度在同一配置行相邻展示。决策卡和排行榜统一使用 `BodyFontFamily`，模型名称、推理强度和分数使用同一套品牌感知大小写规范化显示；例如 `gpt-5.6-sol` 显示为 `GPT5.6-Sol`、`claude-opus-5` 显示为 `Claude-Opus-5`、`xhigh` 显示为 `XHigh`。规范化只作用于展示层，不得回写发布方数据。
...
### 迁移策略
- 使用版本号 `Settings["DatabaseVersion"]` 跟踪数据库版本
- 每次应用启动检查版本，执行必要的 ALTER TABLE 操作
- 保持向后兼容性
- 只有数据库文件不存在且连接后确认没有任何用户表时才按全新数据库初始化；已经存在 `Settings` 或其它业务表但缺失／空白 `DatabaseVersion` 时，必须先验证完整已知 schema 并映射到明确的历史版本，否则停止启动。不得把未知既有数据库标记为当前版本。
- 全新数据库的 `ModuleSettings` 必须由 `DashboardModuleCatalog.CreateDefaultModules()` 这一唯一目录来源序列化，默认只启用时间天气、硬件监视、待办事项和快速便签；不得在 SQL 初始化中维护另一份手写 JSON。
- `LumiDesk` 文件迁移只在目标 `UniDesk.db` 尚不存在时执行；目标数据库已存在即视为用户已完成迁移或已使用新版，不得因遗留旧目录中的锁定或损坏文件阻止现有 UniDesk 启动。首次迁移必须把旧库复制到目标目录的唯一暂存文件，使用 SQLite 打开并运行 `quick_check`、已知表／列与版本语义校验，关闭所有句柄后再以同卷原子移动发布为 `UniDesk.db`。校验、关闭或移动任一步失败时删除本次暂存文件、写诊断并停止启动，不得留下半复制目标或生成新空数据库掩盖旧数据。
...
| `DataFontFamily` | 内嵌 JetBrains Mono；中文回退内嵌 Source Han Sans SC | 12-13/18，Medium | 仅用于确需逐字符等宽对齐的诊断数据；模型雷达由 Grid 对齐并统一使用 `BodyFontFamily` |
...
- 城市切换使用同一取消与 generation 契约；切换后的旧城市响应即使网络层无法及时中断，也不得保存缓存或成为当前返回值
