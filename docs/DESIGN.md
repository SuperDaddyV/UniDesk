...
- 维护工具只能执行固定的 `install-or-repair`、`remove-service`、`cleanup-startup` 和 `health-check` 操作，不接收任意路径、命令或脚本；它使用参数数组调用系统工具，并将 UniDesk 专属服务配置为 `Automatic`、`LocalSystem` 和限定恢复策略。迁移清理由同一个固定 `cleanup-startup` 命令读取新安装目录内、由提权安装器写入且受 ACL 保护的固定迁移标记，不接受命令行旧路径。传给 `sc.exe create/config` 的 `binPath=` 值必须在服务注册值中保留包围可执行文件路径的双引号，即使安装目录包含空格也不得产生未加引号的 `LocalSystem` 服务路径。PawnIO 已存在时只确认并启动驱动，不得重复执行安装器；仅在驱动不存在时才校验安装包固定哈希和 Authenticode 签名并执行首次安装。
- 安装、覆盖安装和卸载在停止、重配或删除 `UniDeskHardwareService` 前，必须读取服务注册的 `ImagePath` 并规范化为可执行文件路径；仅当它严格等于当前 `{app}\HardwareService\UniDesk.HardwareService.exe` 时才视为本产品拥有。不存在时安装流程可以创建、卸载流程视为已清理；读取失败或同名服务指向其它路径时必须返回稳定的所有权冲突退出码，且安装器不得使用 `sc.exe` 兜底接管或删除。卸载已拥有服务时必须先禁用服务，再请求停止、以服务名过滤终止残留服务进程并轮询确认服务确已停止或不存在，最后才请求删除；只有确认进程停止且删除已接受／服务已不存在时才能报告成功。
...
- 整个安装目录必须严格为当前系统 `{autopf}\UniDesk`（系统 Program Files 可位于非系统盘），不因取消完整硬件组件而允许自定义用户可写目录。安装器必须显示安装位置页供用户确认和理解限制，但目录输入框与浏览按钮保持只读／禁用；不得隐藏页面后又提示用户返回修改位置。安装器禁用旧安装目录继承，并在 `PrepareToInstall` 再次强制同路径，防止静默参数绕过；原因不仅是 LocalSystem 服务载荷，也包括管理员卸载器和 `requireAdministrator` 修复工具不能位于可整体替换的目录。安装器必须在复制文件、创建管理员卸载器或执行 `{app}` 内维护工具之前，使用受信系统组件对系统 `Program Files` 这一直接受保护边界执行 fail-closed ACL 预检：该目录不得是重解析点，所有者只能是 `SYSTEM`／`Administrators`／`TrustedInstaller`，且普通用户不得拥有删除子项、删除目录、改 ACL 或取得所有权的有效权限；不得继续向上扫描卷根，因为卷根的宽泛但受子目录继承规则隔离的 ACL 不能证明 `Program Files` 可被替换。检测失败时停止安装且不得尝试修改宽泛父目录 ACL。预检路径必须绑定安装器实际解析的精确 `{autopf}`，由提权安装进程覆盖一个专用子进程环境值后传给固定系统 PowerShell；不得信任调用方可预置的通用 `ProgramFiles` 环境变量。维护工具在注册服务前必须复核同一直接受保护边界以及应用／服务载荷自身的 ACL，不得因卷根 ACL 单独拒绝安全的默认安装目录。
- 同一 `AppId` 的旧 `InstallLocation` 位于非 `{autopf}\UniDesk` 目录时，新签名安装器必须接管安全迁移，而不得引导或执行旧目录中可被替换的 `unins000.exe`：旧路径只能从固定 HKLM 同 AppId 卸载注册项读取，必须规范化并拒绝驱动器根、Windows／Program Files／Program Files (x86)／ProgramData／用户配置文件等宽泛目录；若严格 owned 的旧硬件服务存在，先把它设为禁用、停止并删除（删除暂未完成时至少保证已禁用且确认服务进程已停止）；将旧路径写入新受保护安装目录内的固定迁移标记，再由新维护工具只清理当前已加载用户配置单元和计划任务中严格指向该旧目录且文件名匹配的启动项。随后安装到安全位置并覆盖同 AppId 卸载注册。旧目录不得自动执行或删除；安装成功后只提示用户可在资源管理器中手动删除旧普通程序目录，用户数据不受影响。ImagePath 不匹配旧注册目录时视为外部同名服务，绝不接管；命令行不得传入通用旧路径参数。
...
- 在复制或执行管理员维护载荷、注册或启动服务之前，安装器必须将 `{app}` 及硬件服务、修复工具、PawnIO 子目录收紧为仅 `SYSTEM`／`Administrators` 可修改、普通用户只读执行，并移除弱父目录继承的 `Authenticated Users`／`Users` 写权限。递归 ACL 收紧只能作用于全新空目录或已由同一 `AppId` 的 UniDesk 卸载注册项确认的原安装目录；必须拒绝驱动器根、Windows／Program Files／Program Files (x86)／ProgramData／用户配置文件等宽泛系统目录，以及非空但无法确认属于 UniDesk 的目录。目标目录本身或已有子项包含重解析点时也必须拒绝递归，避免 junction／符号链接把 ACL 重写扩展到安装目录之外。维护工具在注册服务前必须再次验证服务 EXE、承载业务代码的 DLL、配置与父目录均不可被普通用户写入；校验失败时拒绝注册并返回稳定退出码。
...
- **导入**：
  ...
  8. 旧备份中的 `WeatherApiKey` 与 `WeatherApiHost` 一律忽略；剪贴板正文写入前使用 DPAPI 保护；任一步失败回滚全部修改，成功后刷新缓存和界面
