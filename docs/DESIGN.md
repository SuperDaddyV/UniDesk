...
- 整个安装目录必须严格为当前系统 `{autopf}\UniDesk`（系统 Program Files 可位于非系统盘），不因取消完整硬件组件而允许自定义用户可写目录。安装器隐藏目录选择页、禁用旧安装目录继承，并在 `PrepareToInstall` 再次强制同路径，防止静默参数绕过；原因不仅是 LocalSystem 服务载荷，也包括管理员卸载器和 `requireAdministrator` 修复工具不能位于可整体替换的目录。安装器必须在复制文件、创建管理员卸载器或执行 `{app}` 内维护工具之前，使用受信系统组件对目标父目录到卷根的祖先链执行 fail-closed ACL 预检：祖先不得是重解析点，所有者只能是 `SYSTEM`／`Administrators`／`TrustedInstaller`，且普通用户不得拥有删除子项、删除目录、改 ACL 或取得所有权的有效权限；检测失败时停止安装且不得尝试修改宽泛父目录 ACL。预检路径必须绑定安装器实际解析的精确 `{autopf}`，由提权安装进程覆盖一个专用子进程环境值后传给固定系统 PowerShell；不得信任调用方可预置的通用 `ProgramFiles` 环境变量。维护工具在注册服务前必须复核同一祖先链约束。
