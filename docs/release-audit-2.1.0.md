...
- TDD 过程包含针对非法备份、四个异常 IPC 客户端、目录枚举错误、失效启动项迁移、设置写入顺序、天气迟到完成、原子待办更新、图标原子发布／内容所有权、修复进程取消语义、未签名门禁和日志隔离的回归测试；覆盖安装与失效启动项两个后续回归加入后，当前工作区已使用仓库钉住的 SDK `10.0.302` 完成 `590/590` 通过，`0` 跳过，GitHub CI 仍须在正式提交上重新验证。五个关键并发／竞态用例此前连续运行 `10/10` 轮，每轮 `5/5` 通过。
