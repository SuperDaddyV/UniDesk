...
本指南适用于未来选择签名发布时的 UniDesk 自有 EXE、实际承载托管代码的 DLL 和最终安装包；签名私钥由 SignPath 的 HSM 管理，不进入本仓库、GitHub 日志或开发电脑。项目所有者明确批准的 `v2.1.0` 未签名正式版例外不使用本流程，也不得表述为 SignPath 已通过。
