...
Official installers are published only through [GitHub Releases](https://github.com/SuperDaddyV/UniDesk/releases). Signed candidates must pass the controlled SignPath and expected-signer checks. The separately approved unsigned `v2.1.0`, `v2.2.0`, and `v2.2.1` exceptions must instead pass the unsigned-readiness gate, exact source-revision and SHA-256 verification, repository CI, and the complete applicable release matrix, and must state `Authenticode: NotSigned` with a concise note that Windows may show a SmartScreen or enterprise-policy prompt. These exceptions apply only to the named versions and do not authorize later unsigned releases.
